...
```bash
npm install cyber-account-plugins permissionless
```
...
#### Signer 
`cyber-account-plugins` relies on `permissionless.js` to get the smart account signer, it provides two utility functions:
